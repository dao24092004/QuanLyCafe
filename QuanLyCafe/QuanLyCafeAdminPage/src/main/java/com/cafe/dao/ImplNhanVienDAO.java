...
public class ImplNhanVienDAO implements INhanVienDAO {
	...
	public List<NhanVien> selectAll() {
		...
		try {
			...
			lst = ss.createQuery("from NhanVien order by id").list();
	...
	public NhanVien selectByid(int id) {
		...
		try {
			...
			nv = (NhanVien) ss.createQuery("from NhanVien where id = :manv").setParameter("manv", id).getSingleResult();
	...
	public boolean update(NhanVien nv ,String msgerr) {
		...
		try {
			...
			System.out.println("Lỗi" + e.getMessage());
			msgerr = e.getMessage();
			System.out.println("update: "+ msgerr);
			ss.getTransaction().rollback();// Hoàn tác lệnh
		} finally {
			ss.close();
		}
	...
	public boolean delete(int id) {
		...
		try {
			// Mở phiên làm việc
			ss.beginTransaction();
			NhanVien nv = ss.get(NhanVien.class, id);
			// Thực hiện lệnh /giao dịch
			ss.delete(nv);
			// Thực hiện xác nhận thay đổi csdl
			ss.getTransaction().commit();
	...
	public List<NhanVien> search(String search) {
	    ...
	    try {
	        ss.beginTransaction();
	        // Sử dụng hàm LOWER để chuyển đổi chuỗi ho_va_ten thành chữ thường
	        lst = ss.createQuery("from NhanVien WHERE LOWER(ho_va_ten) LIKE :search order by id")
	...
	public boolean isUsernameAndPasswordValid(String username, String passwords) {
		 Session session = sf.openSession();
		    ...
		    try {
		        ...
		        Query query = session.createQuery("SELECT COUNT(*) FROM NhanVien WHERE username = :username AND passwords = :passwords AND id_phan_quyen = 1");
